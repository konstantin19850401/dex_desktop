...
//                criticals = d.getQuery("select signature, cvalue from `criticals` where cname = 'MSISDN'");
            ...
            try
            {
                ...
                string sql = "select icc, date_own, owner_id from `um_data` where status > 0";
                if (cbUnit.SelectedItem != null &&
            ...
            dt.Columns.Add("icc", typeof(string));
            ...
            try
            {
                foreach (string s in simdata)
                {
                    ...
                    DataRow[] rr = umdata.Select(string.Format("icc = '{0}'", s));
...
//                                r["docdate"] = doc["docdate"].Text;
        ...
        private void bExportClipboard_Click(object sender, EventArgs e)
        {
            ...
            {
                string csv = "\"ICC\";\"Отделение\";\"Дата отгрузки\";\"Дата договора\"\n" +
        ...
        private void bReport_Click(object sender, EventArgs e)
        {
            if (dt != null && dt.Rows.Count > 0)
            {
                ...
                rv.LocalReport.ReportEmbeddedResource = "DEXPlugin.Report.Yota.DocDates.DocDates.rdlc";
