...
namespace DEXPlugin.Journalhook.Yota.SenderProfile
{
    public partial class ProfileEditForm : Form
    {
        ...
        void refresh()
        {
            ...
            DataTable t = ((IDEXData)toolbox).getTable("yota_profiles");
        ...
        private void bNew_Click(object sender, EventArgs e)
        {
            ...
            if (pee.ShowDialog() == DialogResult.OK)
            {
                ...
                d.runQuery("insert into `yota_profiles` (pname, pcode) values ('{0}', '{1}')", d.EscapeString(pee.tbPname.Text), d.EscapeString(pee.tbPcode.Text));
                refresh();
            }
        }
        ...
        private void bEdit_Click(object sender, EventArgs e)
        {
            if (lbProfiles.SelectedItem != null && lbProfiles.SelectedItem is StringTagItem)
            {
                ...
                {
                    IDEXData d = (IDEXData)toolbox;
                    d.runQuery("update `yota_profiles` set pname = '{0}', pcode = '{1}' where pcode = '{2}'", 
        ...
        private void bDelete_Click(object sender, EventArgs e)
        {
            if (lbProfiles.SelectedItem != null && lbProfiles.SelectedItem is StringTagItem)
            {
                ...
                {
                    IDEXData d = (IDEXData)toolbox;
                    d.runQuery("delete from `yota_profiles` where pcode = '{0}'", d.EscapeString(ste.Tag));
                    refresh();
                }
            }
        }
