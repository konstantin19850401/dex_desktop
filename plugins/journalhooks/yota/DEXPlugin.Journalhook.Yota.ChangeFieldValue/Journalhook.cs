...
namespace DEXPlugin.Journalhook.Yota.ChangeFieldValue
{
    public class Journalhook: IDEXPluginJournalhook
    {
        public string ID
        {
            get
            {
                return "DEXPlugin.Journalhook.Yota.ChangeFieldValue";
            }
        }
        public string Title
        {
        ...
        public Dictionary<string, string> getVisibleSubFunctionsList(string FunctionName)
        {
            ...
            ret["AddrPhone"] = "Контактный телефон";
            ...
            try
            {
                ...
                if ( FunctionName.Equals("changefieldvalue") && docId.StartsWith("DEXPlugin.Document.") )
                {
                    if ( selectedField == null || selectedFieldValue == null )
                    {
                        ...
                        else if ( "AddrPhone".Equals(SubFunctionName) )
                        {
                            ltitle = "Контактный телефон";
                            ltype = "s";
                        }
                        else if ("SellerId".Equals(SubFunctionName))
                        {
                            ltitle = "Код продавца";
                            ltype = "s";
                        }
