...
namespace DEXPlugin.Journalhook.Mega.ChangeTplanGF
{
    public class Journalhook: IDEXPluginJournalhook
    {
        public string ID
        {
            get
            {
                return "DEXPlugin.Journalhook.Mega.ChangeTplanGF";
            }
        }
        public string Title
        {
            get
            {
                return "Изменение значений тарифного плана для GF";
            }
        }
        ...
        public string Description
        {
            get
            {
                return "Изменяет тарифный план для GF в выделенных документах";
            }
        }
        ...
        public Dictionary<string, string> getVisibleFunctionsList(object toolbox)
        {
            ...
            if ( hookVisible )
                ret["changefieldvalue"] = "Изменить тарифный план GF";
            return ret;
        }
                ...
                if ( FunctionName.Equals("changefieldvalue") && docId.StartsWith("DEXPlugin.Document.") )
                {
                    if ( selectedField == null || selectedFieldValue == null )
                    {
                        ...
                        else if ( "DealerCode".Equals(SubFunctionName) )
                        {
                            ...
                            */
                        if ( "Plan".Equals(SubFunctionName) )
                        {
                    ...
                    if ("Plan".Equals(selectedField)) 
                    {
                       //SimpleXML xml = SimpleXML.LoadXml(doc.documentText);
                        ...
                        DataTable dt = d.getQuery("select plan_id, title from `um_plans_gf` where plan_id=" + selectedFieldValue + " order by title");
                        string title = dt.Rows[0]["title"].ToString();
                        string plan_id = dt.Rows[0]["plan_id"].ToString();

                        xml["Plan"].Text = plan_id;
                        xml["IntPlanPrn"].Text = title;
