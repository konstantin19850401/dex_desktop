...
namespace DEXPlugin.Journalhook.Mega.CheckOutSim
{
    public class Journalhook : IDEXPluginJournalhook
    {
        public string ID
        {
            get
            {
                return "DEXPlugin.Journalhook.Mega.CheckOutSim";
            }
        }
        public string Title
        {
        ...
        public bool RunFunctionForDocument(string FunctionName, string SubFunctionName, string docId, IDEXDocumentData doc, out bool docChanged)
        {
            ...
            try
            {
                    ...
                    journalType == DEXJournalType.JOURNAL)
                {
                    ...
                    DataTable t = d.getQuery(string.Format("select * from `um_data` where msisdn = '{0}'",
                    ...
                    else
                    {
                        ...
                        d.runQuery(
                            "insert into `um_data` (status, status_j, msisdn, icc, fs, billing, date_in, owner_id, date_own, " +
                            "date_sold, region_id, party_id, plan_id, balance, comment, data) values (2, 0, '{0}', '{1}', 0, 0, '{2}', " +
                            "{3}, '{4}', '{5}', '-', 1, '-', '100','','')",
