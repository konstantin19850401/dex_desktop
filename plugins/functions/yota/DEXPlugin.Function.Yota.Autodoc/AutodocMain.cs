    ...
    public partial class AutodocMain : Form
    {
        ...
        public AutodocMain(object toolbox)
        {
            try
            {
                ...
                DataTable td = ( (IDEXData)toolbox ).getTable("um_regions");
                StringTagItem.UpdateCombo(cbRegion, td, null, "region_id", "title", false);
                ...
                if (srcdt != null && srcdt.Rows.Count > 0 && srcdt.Columns.Count > 1)
                {
                    ...
                    foreach (DataRow r in srcdt.Rows)
                    {
                        ...
                        if (!msichk.IsMatch(msi))
                        {
                            ...
                            fer += "Некорректный ICC; ";
                        }
                        else
                        {
                            DataTable ticc = d.getQuery(
                                string.Format("select icc, status, plan_id, owner_id from `um_data` where icc = '{0}'",
                                d.EscapeString(msi))
                                );
        ...
        public string BuildDocsVsMsisdn(IWaitMessageEventArgs wmea)
        {
            ...
            try
            {
                ...
                string women = "";
                if (cbOnlyWomen.Checked) women = "AND data LIKE '%Sex=2%'";
                

                DataTable tb = d.getQuery("SELECT phash, data, lastdate, usedcount, unitid " +
                    "FROM `autodoc_people` left join `autodoc_people_usage` using (phash) where docid = 'DEXPlugin.Document.Yota.Contract' "+women+" order by usedcount, lastdate");
                ...
                if (tb != null && tb.Rows.Count > 0)
                {
                    ...
                    foreach (DataRow r in tb.Rows)
                    {
                        ...
                        string region = "";
                        bool fullData = false;
                        // отсеим неудовлетворяющих условию налия всех данных
                        if ( dd["FirstName"] != "" && dd["SecondName"] != "" && dd["LastName"] != "" && dd["Birth"] != "" && dd["FizBirthPlace"] != "" && dd["FizDocSeries"] != ""
                            && dd["FizDocNumber"] != "" && dd["FizDocDate"] != "" && dd["FizDocType"] != "" && dd["FizDocOrg"] != "" && dd["FizDocOrgCode"] != "" && dd["Sex"] != "" && dd["AddrState"] != "" && dd["AddrPhone"] != ""
                            ) 
                        {
                            // а теперь отсеим если паспорт просрочен
                            bool statusPassport = true;
                            IDEXData ddata = (IDEXData)toolbox;
                            try
                            {
                                if (ddata.AccessRemoteServer)
                                {
                                    ...
                                    sb.Replace("{$db_server$}", ddata.PasspHostDb);
                                    sb.Replace("{$db_name$}", ddata.PasspNameDb);
                                    sb.Replace("{$db_user$}", ddata.PasspUserDb);
                                    sb.Replace("{$db_pass$}", ddata.PasspPassDb);
                                    conn = sb.ToString();
                                    ...
                                    cmd.Parameters.AddWithValue("@number", Convert.ToInt64(dd["FizDocSeries"] + dd["FizDocNumber"]));
                                    cmd.ExecuteNonQuery();
                                ...
                                {
                                    DataTable tu = d.getQuery("select SQL_CALC_FOUND_ROWS value from `list_of_expired_passports` where value='{0}'", Convert.ToInt64(dd["FizDocSeries"] + dd["FizDocNumber"]));
                            ...
                            if (statusPassport) fullData = true;
                        }
                        //if ( dd["LastName"] == "Семенцов")
                        //{
                        //    string ss = "dd";
                        //}
                        if ( dd.ContainsKey("DocReg") )
                        {
                            region = dd["DocReg"];
                        }
                        else
                        {
                            region = StringTagItem.getSelectedTag(cbRegion, "");
                        }
            ...
            try
            {
                foreach (DataRow r in dtSrc.Rows)
                {
                    if (r["error"].ToString().Equals(""))
                    {
                        ...
                        if ( abdata["DocReg"] == "") {
                            if ( abdata["AddrRegion"] == "" && cbRegion.SelectedIndex != -1)
                            {
                                //xml["DocReg"].Text = cbRegion.SelectedIndex.ToString();
                                xml["DocReg"].Text = StringTagItem.getSelectedTag(cbRegion, "");
                                
                            }
                            else
                            {
                                xml["DocReg"].Text = abdata["AddrRegion"].ToString();
                            }
                        } else
                        {
                            xml["DocReg"].Text = abdata["DocReg"];
                        }
                        ...
                        string digest = string.Format(
                            "{0}, {1}, {2} {3}",
                            xml["ICC"].Text, xml["LastName"].Text,
