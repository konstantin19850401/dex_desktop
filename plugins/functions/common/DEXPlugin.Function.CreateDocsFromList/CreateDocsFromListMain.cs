...
*/
        ...
        public CreateDocsFromListMain(object toolbox)
        {
            ...
            i = dgvSrc.Columns.Add("plan", "Тарифный план");
            dgvSrc.Columns[i].DataPropertyName = "plan";

            i = dgvSrc.Columns.Add("dealerCode", "Код дилера");
            ...
            try
            {
                ...
                if (srcdt != null && srcdt.Rows.Count > 0 && srcdt.Columns.Count > 1)
                {
                    ...
                    foreach (DataRow r in srcdt.Rows)
                    {
                        ...
                        nr["icc"] = r["field1"].ToString();
                        nr["docDate"] = r["field2"].ToString();
                        nr["plan"] = r["field3"].ToString();
                        nr["dealerCode"] = r["field4"].ToString();
                        nr["description"] = "Отсутствует в базе. Договор будет создан";
                        DataTable msisdnDt = d.getQuery("SELECT * FROM criticals WHERE cvalue = '" + r["field0"].ToString() + "'");
                        DataTable iccDt = d.getQuery("SELECT * FROM criticals WHERE cvalue = '" + r["field1"].ToString() + "'");
        ...
        private void bMakeDocs_Click(object sender, EventArgs e)
        {
            ...
            {
                if (dr["description"].ToString().Equals("Отсутствует в базе. Договор будет создан"))
                {
                    cnt++;
                    string unitid = "";
                    IDEXData d = (IDEXData)toolbox;
                    ...
                    if (cbFixedDocDate.Checked) 
                    {
                        try
                        {
                            if (!rxdate.IsMatch(deDocDate.Text))
                            {
                                break;
                            }
                            else
        ...
        private void dgvSrc_RowPrePaint(object sender, DataGridViewRowPrePaintEventArgs e)
        {
            ...
            if (cv.Equals("Отсутствует в базе. Договор будет создан"))
            {
