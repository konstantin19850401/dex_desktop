...
//using System.IO; // убрать

namespace DEXPlugin.Function.DistributeSIM
{
    public partial class DistributeSimMain : Form
    {
        ...
        void _select()
        {
            ...
            string[] ordt = { "msisdn", "icc", "party_id", "date_in", "plan_id" , "region_id"};
            ...
            try
            {
                ...
                if (dt != null && dt.Rows.Count > 0)
                {
                    foreach (DataRow r in dt.Rows)
                    {
                        ...
                        string rrr = r["region_id"].ToString();

                        SimItem snew = new SimItem(
                            r["id"].ToString(), FormatRow(r), r["msisdn"].ToString(), r["icc"].ToString(),
                            dp, r["party_id"].ToString(), pln,
                            false, rrr);
        ...
        private void bDistributeFromFile_Click(object sender, EventArgs e)
        {
            if (ofd.ShowDialog() == DialogResult.OK && File.Exists(ofd.FileName))
            {
                try
                {
                    ...
                    try
                    {
                        sd = d.getQuery("select id, msisdn, icc, owner_id, date_own from `um_data` where auto_distr = 1 AND ( status = 0 or status = 1 )");
                    } catch(Exception ex) 
                    {
                        sd = d.getQuery("select id, msisdn, icc, owner_id, date_own from `um_data` where status = 0 or status = 1");
                    }
                    if (sd == null)
                    {
                        sd = d.getQuery("select id, msisdn, icc, owner_id, date_own from `um_data` where status = 0 or status = 1");
                    }
                    //DataTable sd = d.getQuery("select id, msisdn, icc, owner_id from `um_data` where status = 0 or status = 1");
                    ...
                    try
                    {
                        ...
                        if (File.Exists(path + @"\logs\distribute_" + dtime.ToString("ddMMMMyyyy") + ".txt"))
                        {
                            File.Delete(path + @"\logs\distribute_" + dtime.ToString("ddMMMMyyyy") + ".txt");
                        }
                        
                    } catch (Exception) 
                    {
                    }
                    StreamWriter textFile = new StreamWriter(path + @"\logs\distribute_" + dtime.ToString("ddMMMMyyyy") + ".txt");
                    ...
                    if (sd != null && sd.Rows.Count > 0)
                    {
                        ...
                        foreach (string s in ss)
                        {
                            ...
                            try {
                                textFile.WriteLine(s); // убрать логирование
                            }
                            catch (Exception)
                            {
                            }
                            ...
                            {
                                DataRow[] sels = sd.Select("icc = '" + str[1] + "'");
                                if (sels != null && sels.Length > 0)
                                {
                                    // пока обработка на кбр салп из-за 1с
                                    IDEXConfig idc = (IDEXConfig)toolbox;
                                    IDEXUserData ids = (IDEXUserData)toolbox;
                                    string cbase = ids.dataBase;
                                    if (cbase.Equals("dex_mts_kbr_salp") && str[0].Equals("")) 
                                    {
                                        ...
                                        //{
                                            try
                                            {
                                                ...
                                                // убрать логирование
                                                DataTable ttt = d.getQuery("select owner_id from `um_data` where icc='" + sels[0]["icc"].ToString() + "'");
                                                textFile.WriteLine("Бывший владелец " + ttt.Rows[0]["owner_id"].ToString());
                                                ...
                                                // убрать логирование
                                                ttt = d.getQuery("select owner_id from `um_data` where icc='" + sels[0]["icc"].ToString() + "'");
                                                textFile.WriteLine("Новый владелец " + ttt.Rows[0]["owner_id"].ToString() + "\n");
                                            }
                                            catch (Exception)
                                            {
                                                //string dpp = "";
                                    ...
                                    else 
                                    {
                                        if (str[0].Equals(sels[0]["msisdn"].ToString()))
                                        {
                                            try
                                            {
                                                ...
                                                // убрать логирование
                                                DataTable ttt = d.getQuery("select owner_id from `um_data` where msisdn='" + sels[0]["msisdn"].ToString() + "'");
                                                textFile.WriteLine("Бывший владелец " + ttt.Rows[0]["owner_id"].ToString());
                                                ...
                                                // убрать логирование
                                                ttt = d.getQuery("select owner_id from `um_data` where msisdn='" + sels[0]["msisdn"].ToString() + "'");
                                                textFile.WriteLine("Новый владелец " + ttt.Rows[0]["owner_id"].ToString() + "\n");
                                            }
                                            catch (Exception)
                                            {
                        ...
                        textFile.Close();  // убрать логирование
        ...
        private void bPrintUnassigned_Click(object sender, EventArgs e)
        {
            ...
            if (dtsrc != null && dtsrc.Rows.Count > 0)
            {
                ...
                rv.LocalReport.ReportEmbeddedResource = "DEXPlugin.Function.DistributeSIM.SimList.rdlc";
    ...
    {
        public string id, text, msisdn, icc, date, party, plan, region;
        public bool ch;

        public SimItem(string aid, string atext, string amsisdn, string aicc, string adate, string aparty, string aplan, bool ach, string aregion)
        {
            id = aid;
            text = atext;
            ch = ach;
