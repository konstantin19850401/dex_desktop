                ...
                if (cnt > 0)
                {
                    ...
                    foreach (DataRow r in dt.Rows)
                    {
                        try {
                            ...
                            string msisdn = xml.GetNodeByPath("MSISDN", false).Text.Trim(), 
                                icc = xml.GetNodeByPath("ICC", false).Text.Trim();

                            if (msisdn.Length > 0 && icc.Length > 0)
                            {
                                data.runQuery("update `um_data` set icc = '{0}', owner_id = {1} where msisdn = '{2}'", icc, unitid, msisdn);
                            }
                        } catch (Exception) { }
