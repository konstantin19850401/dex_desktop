...
//            nudPartyNum.Value = cfg.getInt(this.Name, "nudPartyNum", 1);
            ...
            cbDublicateIcc.SelectedIndex = cfg.getInt(this.Name, "cbDublicateMsisdn", 0);
...
//            cfg.setStr(this.Name, "nudPartyNum", nudPartyNum.Value.ToString());
            ...
            cfg.setInt(this.Name, "cbDublicateMsisdn", cbDublicateIcc.SelectedIndex);
...
//                        dgvc.AutoSizeMode = DataGridViewAutoSizeColumnMode.AllCellsExceptHeader;
                    ...
                    foreach(string ln in strs)
                    {
                        ...
                        if (rer == "")
                        {
                            ...
                            foreach (KeyValuePair<string, object> kvp in flds)
                            {
                                ...
                                if (kvp.Key == "icc") 
                                {
                                    string sss = r[kvp.Key].ToString();
                                    r[kvp.Key] = sss.Substring(Convert.ToInt32(nuIccStartPosition.Value));
                                }
                            ...
                            if (chkdb)
                            {
                                ...
                                if (t2 != null && t2.Rows.Count > 0)
                                {
                                    ...
                                    {
                                        /*
                                        if (r2["icc"].ToString().Equals(r["icc"].ToString()))
                                        {
                                            emsi = true;
        ...
        private void bOk_Click(object sender, EventArgs e)
        {
            ...
            int typeSim = cbTypeSim.SelectedIndex;
            if ( typeSim == -1 ) 
            {
                MessageBox.Show("Вы не выбрали тип загружаемых сим-карт!");
                er += "Вы не выбрали тип загружаемых сим-карт";
            }
                    ...
                    "Продолжить?", "Подтверждение", MessageBoxButtons.YesNo) == DialogResult.Yes)
                {
                    ...
                    foreach (DataRow r in t.Rows)
                    {
                        if (r["status"].ToString().Trim().Equals("") && (r["old"].ToString().Trim().Equals("") || cbOldToCurrent.Checked))
                        {
                            ...
                            {
                                if (cbDublicateIcc.SelectedIndex == 0)
                                {
                                    dtb.runQuery(string.Format(
                                        "insert into `um_data_out` (status, msisdn, icc, type_sim, fs, date_in, owner_id, date_own, date_sold, " +
                                        "region_id, party_id, plan_id, balance, comment, data) SELECT status, msisdn, icc, type_sim, fs, date_in, owner_id, " +
                                        "date_own, date_sold, region_id, party_id, plan_id, balance, comment, data FROM `um_data` WHERE icc = '{0}'",
                                        r["icc"].ToString()));
                                }

                                dtb.runQuery(string.Format(
                                    "delete from `um_data` where icc = '{0}'",
                                    r["icc"].ToString()));
                            }
