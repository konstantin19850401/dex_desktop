...
namespace DEXPlugin.Dictionary.Yota.Sim
{
    public partial class FSimMain : Form
    {
        ...
        string[] typeSim = new string[5] {"Голос", "Модем", "Для модема" , "Саморегистрация", "Голос мультиточка"};
        ...
        public void InitForm()
        {
            ...
            this.type_sim.DataPropertyName = "type_sim";
...
//            MessageBox.Show("sqlr:\n\n" + sqlr + "\n\nsqlc:\n\n" + sqlc + "\n\nwhr:\n\n" + whr);
        ...
        private void dgvSim_CellFormatting(object sender, DataGridViewCellFormattingEventArgs e)
        {
            ...
            else if ( fcol.Equals("type_sim") ) 
            {
                ...
                {
                    e.Value = "Саморегистрация";
                }
                else if (Convert.ToInt32(e.Value) == 4)
                {
                    e.Value = "Голос мультиточка";
                }

                //e.Value = Convert.ToInt32(e.Value) == 0 ? "Голос" : "Модем";
            }
        ...
        private void tsbNewSim_Click(object sender, EventArgs e)
        {
            ...
            if (simEd.ShowDialog() == DialogResult.OK)
            {
                ...
                string sql =
                    "insert into `" + simJournalName[simType] + "` (status, msisdn, icc, tipe_sim, fs, date_in, owner_id, " +
                    "date_own, date_sold, region_id, party_id, plan_id, balance, comment) values (" +
                    "@status, @msisdn, @icc, @type_sim, @fs, @date_in, @owner_id, @date_own, " +
                ...
                */
                ...
                bsSim.Position = bsSim.Find("icc", simEd.tbICC.Text);
        ...
        private void tsbEditSim_Click(object sender, EventArgs e)
        {
            ...
            if (simEd.ShowDialog() == DialogResult.OK)
            {
                ...
                string sql =
                    ...
                    "icc = @icc, fs = @fs, date_in = @date_in, owner_id = @owner_id, " +
                ...
                if (dt != null && dt.Rows.Count > 0)
                {
                    ...
                    if (opt > -1)
                    {
                        ...
                        foreach (DataRow r in dt.Rows)
                        {
                            ...
                            if (cst == 0 || opt == 0)
                            {
                                string nmsi = r["msisdn"].ToString();
                                string nicc = "";
                                if ( Convert.ToInt32(nudIccAddZero.Value) != 0 )
                                {
                                    for ( int i = 0; i < Convert.ToInt32(nudIccAddZero.Value); i++ )
                                    {
                                        nicc += "0";
                                    }
                                    nicc += r["icc"].ToString();
...
//                                    nmsi = "+7 " + nmsi.Substring(0, 3) + " " + nmsi.Substring(3, 3) + "-" + nmsi.Substring(6, 2) + "-" + nmsi.Substring(8, 2);
        ...
        private void tsmiChangePartyId_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                Form1 form = new Form1(toolbox, scpid, false);
        ...
        private void tsmiSChangePartyId_Click(object sender, EventArgs e)
        {
            try
            {
                if (dgvSim.SelectedRows != null && dgvSim.SelectedRows.Count > 0)
                {
                    Form1 form = new Form1(toolbox, 0, true);
                    if (form.ShowDialog() == DialogResult.OK)
                    {
        ...
        private void tsmiSCopyToClipboard_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (dgvSim.SelectedRows != null && dgvSim.SelectedRows.Count > 0)
                {
                    ...
                    string dta = "Карта продана\tMSISDN\tICC\tФС\tПартия\tОтделение\tПоступление\tВыдача\tПродажа\tТП\tБаланс\tРегион\tПримечание\n";
        ...
        public string PrepareOstatki(IWaitMessageEventArgs wmea)
        {
            ...
            try
            {
                ...
                //string sql =
                //    "select count(ud.id) as cnt, ud.plan_id as plan_id, un.title as title, ud.fs " +
                //    "from `um_data` as ud, `units` as un " +
                //    "where ud.status = 1 and un.uid = ud.owner_id " +
                //    "group by ud.plan_id, un.title";
                string sql =
                    "select count(ud.id) as cnt, un.title as title, un.region as region " +
                    "from `um_data` as ud, `units` as un " +
                    "where ud.status = 1 and un.uid = ud.owner_id " +
                    "group by un.title";
                ...
                if ( dtOstatki != null && dtOstatki.Rows.Count > 0 )
                {
                    ...
                    {
                        //if ( r["plan_id"].ToString().Trim().Equals("") )
                        //    r["plan_id"] = "-";
                    ...
                    //if ( !doDetailedOstatki )
                        dtOstatki.Rows.Clear();
                    ostatkiCnt = 0;
        ...
        private void tsmiReportOstatki_Click(object sender, EventArgs e)
        {
            ...
            if ( ret.Equals("") )
            {
                ...
                if (doRegions)
                {
                    rv.LocalReport.ReportEmbeddedResource = "DEXPlugin.Dictionary.Yota.Sim.OstatkiRegions.rdlc";

                }
                else
                {
                    rv.LocalReport.ReportEmbeddedResource = "DEXPlugin.Dictionary.Yota.Sim.Ostatki.rdlc";
                    ...
                    foreach (DataGridViewRow dgvr in dgvSim.SelectedRows)
                    {
                        ...
                        try
                        {
                            ...
                            sql = string.Format(
                                ...
                                d.EscapeString(dr["data"].ToString()), Convert.ToBoolean(dr["fs"]) ? 1 : 0);
                            d.runQuery(sql);
            ...
            try
            {
                ...
                foreach (KeyValuePair<string, string> sim in dSim)
                {
                    DataTable t = d.getQuery(
                        "select * from `um_data` where icc = '{0}'",
                    ...
                    if (t != null && t.Rows.Count > 0)
                    {
                        foreach (DataRow row in t.Rows)
                        {
                            try
                            {
                                ...
                                {
                                    d.runQuery(
                                        "delete from `um_data_out` where and icc = '{1}'",
                                        d.EscapeString(row["icc"] as string));

                                    d.runQuery(
                                        "insert into `um_data_out` (status, msisdn, icc, date_in, owner_id, " +
                                        "date_own, date_sold, region_id, party_id, plan_id, balance, comment, data, fs) " +
        ...
        private void tsmiChangeTypeSim_Click(object sender, EventArgs e)
        {
            try
            {
                if (dgvSim.SelectedRows != null && dgvSim.SelectedRows.Count > 0)
                {
                    FChangeTypeSim form = new FChangeTypeSim();
                    if (form.ShowDialog() == DialogResult.OK)
                    {
                        IDEXData d = (IDEXData)toolbox;
                        foreach (DataGridViewRow dgvr in dgvSim.SelectedRows)
                        {
                            ...
                            d.runQuery(string.Format(
                            "update `um_data` set type_sim = '{0}' where icc = {1}",
                            form.cbChangeTypeSim.SelectedIndex, d.EscapeString(dr["icc"].ToString())
                            ));
                        }
                    }
                    MessageBox.Show("Изменение типа сим-карты для выделенных завершено");
                    _sim();
                }   
            }
            catch (Exception)
        ...
        private void tsmiChangePartyTypeSim_Click(object sender, EventArgs e)
        {
            DataRowView drv = bsSim[dgvSim.CurrentRow.Index] as DataRowView;
            int scpid = int.Parse(drv.Row["party_id"].ToString());
            FChangeTypeSim form = new FChangeTypeSim();
            if (form.ShowDialog() == DialogResult.OK)
            {
                IDEXData d = (IDEXData)toolbox;
                d.runQuery(string.Format(
                    "update `um_data` set type_sim = '{0}' where party_id = {1}",
                     form.cbChangeTypeSim.SelectedIndex, scpid
                    ));
                MessageBox.Show("Изменение типа сим-карт партии завершено");
                _sim();   
            }
        }
