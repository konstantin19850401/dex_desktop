...
namespace DEXPlugin.Dictionary.Beeline.Sim
{
    public partial class FSimEd : Form
    {
        ...
        public void InitForm(Object AToolBox, string journalName, DataRow ARow)
        {
            ...
            try
            {
                ...
                foreach(DataRow dr in t.Rows)  
                {
                    ...
                    {
                        if (dtdr["uid"].ToString().Equals(dtdr["uid"].ToString()))
                        {
                            dtdr["title"] = dtdr["title"].ToString() + " [" + dtdr["region"].ToString() + "]";
                            break;
                        }
                    }
                }
            ...
            foreach (StringTagItem sti in cbUnit_id.Items) 
            {
                foreach (DataRow dr in t.Rows)
                {
                    if (sti.Tag.Equals(dr["uid"].ToString()))
                    {
                        if (!"".Equals(dr["region"].ToString()))
                        {
                            sti.Text = sti.Text + " [" + dr["region"].ToString() + "]";
                        }
                        break;
                    }
                }
            ...
            else
            {
                ...
                cb_typeSim.SelectedIndex = int.Parse(row["dynamic"].ToString());
        ...
        private void bOk_Click(object sender, EventArgs e)
        {
            ...
            if (cb_typeSim.SelectedIndex < 0)
            {
                er += "* Ну указан тим сим-карты\n";
            }
