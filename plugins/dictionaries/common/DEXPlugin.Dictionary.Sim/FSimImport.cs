        ...
        private void bOk_Click(object sender, EventArgs e)
        {
            ...
            if (er == "")
            {
                    ...
                    "Продолжить?", "Подтверждение", MessageBoxButtons.YesNo) == DialogResult.Yes)
                {
                    ...
                    int impc = 0;
                    int device_type = cbDeviceType.SelectedIndex;
                    ...
                    foreach (DataRow r in t.Rows)
                    {
                        if (r["status"].ToString().Trim().Equals("") && (r["old"].ToString().Trim().Equals("") || cbOldToCurrent.Checked))
                        {
                            if (!r["old"].ToString().Equals(""))
                            {
                                if (cbDublicateMsisdn.SelectedIndex == 0)
                                {
                                    dtb.runQuery(string.Format(
                                        "insert into `um_data_out` (status, msisdn, icc, fs, dynamic, device_type, date_in, owner_id, date_own, date_sold, " +
                                        "region_id, party_id, plan_id, balance, comment, data) SELECT status, msisdn, icc, fs, dynamic, device_type, date_in, owner_id, " +
                            ...
                            string sql = "insert into `um_data` (status, msisdn, icc, fs, dynamic, device_type, date_in, owner_id, date_own, date_sold, region_id, party_id, plan_id, balance, comment, data) values (" +
                                "0," +
                                "'" + dtb.EscapeString(r["msisdn"].ToString()) + "', " +
                                "'" + dtb.EscapeString(r["icc"].ToString()) + "', " +
                                "'" + ifs.ToString() + "', " +
                                "'" + idynamic.ToString() + "', " +
                                "'" + device_type + "', " +
                                "'" + datein + "', " +
                                ...
                                "'', " +
                                "'', " +
                                "'" + dtb.EscapeString(r["region_id"].ToString()) + "', " +
                                "'" + r["party_id"].ToString() + "', " +
                                "'" + dtb.EscapeString(r["plan_id"].ToString()) + "', " +
