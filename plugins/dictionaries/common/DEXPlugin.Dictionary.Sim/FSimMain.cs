...
//                                    nmsi = "+7 " + nmsi.Substring(0, 3) + " " + nmsi.Substring(3, 3) + "-" + nmsi.Substring(6, 2) + "-" + nmsi.Substring(8, 2);
        ...
        DataTable dtOstatki;
        bool doDetailedOstatki, doFsSeparated, doDetailedNull, doRegions, doOnlyActiveUnits;
        int ostatkiCnt;
        ...
        public string PrepareOstatki(IWaitMessageEventArgs wmea)
        {
            ...
            try
            {
                ...
                string sql =
                    "select count(ud.id) as cnt, ud.plan_id as plan_id, un.title as title, un.uid as uid, un.region as region, ud.fs " +
        ...
        private void tsmiReportOstatki_Click(object sender, EventArgs e)
        {
            ...
            if (ret.Equals(""))
            {
                ...
                if (doRegions)
                {
                    rv.LocalReport.ReportEmbeddedResource = "DEXPlugin.Dictionary.Sim.OstatkiRegions.rdlc";

                }
                else
                {
                    rv.LocalReport.ReportEmbeddedResource = "DEXPlugin.Dictionary.Sim.Ostatki.rdlc";
                    ...
                    foreach (DataGridViewRow dgvr in dgvSim.SelectedRows)
                    {
                        ...
                        try
                        {
                            ...
                            sql = string.Format(
                                ...
                                "date_sold, region_id, party_id, plan_id, balance, comment, data, fs, dynamic) values ({1}, " +
                                ...
                                d.EscapeString(dr["data"].ToString()), Convert.ToBoolean(dr["fs"]) ? 1 : 0, dr["dynamic"].ToString() );
                            d.runQuery(sql);
