    ...
    public partial class FUnitsDPEd : Form
    {
        ...
        private void bOK_Click(object sender, EventArgs e)
        {
            ...
            if (er == "")
            {
                ...
                string sql = 
                    "select count(id) as cid from `mts_units_dp` where uid = " + uid + " and dpcode = '" +
                    d.EscapeString(tbDPCode.Text) + "' and kind = " + kind;
            ...
            if (er == "")
            {
                ...
                {
                    d.runQuery("update `mts_units_dp` set uid = " + uid + ", dpcode = '" + dpcode + "', kind = " + kind + " where id = " + row["id"].ToString());
                }
