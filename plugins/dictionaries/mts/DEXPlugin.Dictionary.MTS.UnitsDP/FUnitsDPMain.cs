...
namespace DEXPlugin.Dictionary.MTS.UnitsDP
{
    public partial class FUnitsDPMain : Form
    {
        ...
        public void refresh()
        {
            ...
            bs.DataSource = ((IDEXData)toolbox).getQuery("select mud.*, u.title from `mts_units_dp` mud, `units` u where mud.uid = u.uid");
        ...
        private void bDelete_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (er == "")
                {
                    ...
                    {
                        ((IDEXData)toolbox).runQuery("delete from `mts_units_dp` where id = {0}", ((DataRowView)bs.Current).Row["id"].ToString());
                        refresh();
                    }
                }
                else
