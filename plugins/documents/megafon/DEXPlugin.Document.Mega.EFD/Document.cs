...
namespace DEXPlugin.Document.Mega.EFD
{
    public class Document : IDEXPluginDocument, IDEXPluginSetup
    {
        ...
        public string ID
        {
            get
            {
                return "DEXPlugin.Document.Mega.EFD.Fiz";
            }
        }
        public string Title
        {
            get
            {
                return "МегаФон: Единая форма договора (Физ)";
            }
        }
        ...
        public string Description
        {
            get
            {
                return "Единая форма абонентского договора МегаФон для физ.лиц";
            }
        }
        ...
        public void Startup(Object toolbox)
        {
            ...
            r.AddRightsItem(ID + ".new", "МегаФон ЕФД: Создание документа");
            r.AddRightsItem(ID + ".edit", "МегаФон ЕФД: Изменение документа");
            r.AddRightsItem(ID + ".delete", "МегаФон ЕФД: Удаление документа");
        ...
        public bool CloneDocument(Object toolbox, IDEXDocumentData source, IDEXDocumentData document)
        {
            ...
            if (r.GetRightsItem(ID + ".new") || r.IsSuperUser())
            {
                ...
                form.deJournalDate.Value = new DateTime(int.Parse(now.Substring(0, 4)), int.Parse(now.Substring(4, 2)), int.Parse(now.Substring(6, 2)));
                form.deDocDate.Value = new DateTime(int.Parse(now.Substring(0, 4)), int.Parse(now.Substring(4, 2)), int.Parse(now.Substring(6, 2)));
                if (form.ShowDialog() == DialogResult.OK)
                {
                    string cd_date = "?";
                    string cd_number = "?";
        ...
        public ArrayList ValidateDocument(Object toolbox, IDEXDocumentData document)
        {
            ...
            DataTable dt = d.getQuery("SELECT billing FROM `um_data` where msisdn = '" + xml["MSISDN"].Text + "' and icc = '" + xml["ICC"].Text + "'");
            bool typeSim = false;
            if (dt!= null && dt.Rows.Count > 0)
            {
                ...
                {
                    typeSim = "1".Equals(dt.Rows[0]["billing"].ToString());
                }
            ...
            if (xml["ICC"].Text.Trim().Length < 17) ret.Add("Некорректный ICC");
            ...
            {
                ret.Add("Регистрация возможна только на парпорт гражданина РФ");
            }
            if (xml["DocClientType"].Text != "1")
            {
                ret.Add("Регистрация возможна только на резидента");
            }
            */
            ...
            else
            {
                try
                {
                    ...
                    bool older = true;
                    if (d2.Year - d1.Year < 14) older = false;
                    else if (d2.Year - d1.Year == 14)
                    {
                    ...
                    if (!older) ret.Add("Абонент не должен быть младше 14 лет");

                }
                catch (Exception) {
                    ret.Add("Ошибка в процессе проверки даты рождения абонента");
                }
            }
            ...
            else if ("1".Equals(xml["FizDocType"].Text.Trim()))
            {
                try
                {
                ...
                if ("1".Equals(xml["DocClientType"].Text.Trim()))
                {
                    try
                    {
                        ...
                        */
                        ...
                        bool isOnline = ((IDEXUserData)toolbox).isOnline;
                        string currentBase = !isOnline ? ((IDEXUserData)toolbox).dataBase : ((IDEXUserData)toolbox).currentBase;
                        IDEXServices idis = (IDEXServices)toolbox;
                        ...
                        {
                            JObject packet = new JObject();
                            packet["com"] = "dexdealer.adapters.megafon";
                            ...
                            JObject o = JObject.Parse(idis.sendRequest("GET", nodejsserver, "3020", "/dexdealer/megafon/cmd?packet=" + JsonConvert.SerializeObject(packet) + "&uid=" + adaptersUid + "&clientType=dexol", 1));
            ...
            if (document.documentStatus > 0)
            {
                ...
                string[] pch = null;
                if ("1".Equals(xml["FizDocType"].Text.Trim()))
                {
                    pch = ((IDEXPeopleSearcher)toolbox).checkPassport(sl);
                }
        ...
        public Dictionary<string, string> GetDocumentFields(Object toolbox)
        {
            ...
            ret["DocNum"] = "№ договора";
            ret["customerId"] = "Идентификатор клиента";
...
//-            ret["PlanPrn"] = "ТП";
            ...
            ret["FizDocScanMime"] = "Расширение скана";
        ...
        { // OK
            ...
            sl["FizDocScanMime"] = xml.GetNodeByPath("FizDocScanMime", true).Text;
